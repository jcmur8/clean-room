...
import { memoryAdapter } from "../js/storage.js";
import { setLanguage, t, localized } from "../js/i18n.js";
const tests = [];
function test(n, f) {
  tests.push([n, f]);
}
const base = () => {
  ...
  d.appSettings.setupComplete = true;
  d.children = [
    { id: "a", displayName: "A", active: true, roleRestrictions: [] },
    { id: "b", displayName: "B", active: true, roleRestrictions: [] },
  ];
  return d;
};
test("Role rotation and restrictions", () => {
  const d = base(),
    m = d.missions[0];
  d.children[0].roleRestrictions = ["Finder"];
  const a = assignRoles(d.children, m, 0, d.roles);
  assert(a[0].role !== "Finder");
});
test("Game-mode mission selection", () =>
  equal(selectMissions(base(), "quick").length, 4));
test("Session snapshot stability", () => {
  const d = base(),
    s = createSession(d, "quick", 1000),
    title = s.missionSnapshots[0].title;
...
test("Pause and timer calculations", () => {
  ...
  equal(elapsedMs(t, 5000), 4000);
  t = pauseTimer(t, 5000);
  equal(elapsedMs(t, 9000), 4000);
...
test("Configurable mission countdown reset", () => {
  ...
  st = resumeStepTimer(st, 500000);
  equal(stepRemainingMs(st, 501000), 239000);
});
test("Parent lockout logic model", () => {
  let s = { failedAttempts: 4, lockedUntil: 0 };
  s = { ...s, failedAttempts: s.failedAttempts + 1, lockedUntil: 1000 + 30000 };
  assert(s.lockedUntil === 31000);
});
test("Schema validation", () => equal(validateRoot(base()), []));
test("Import rejection without mutation", () => {
  const before = JSON.stringify(base()),
    bad = { schemaVersion: 99 };
  assert(validateRoot(bad).length > 0);
  equal(JSON.stringify(base()), before);
});
test("Migration from older fixture", () => {
  const d = base();
  d.schemaVersion = 1;
  delete d.rewards;
...
test("Export/import round trip", async () => {
  const a = await memoryAdapter(),
    d = base();
  await a.save(JSON.parse(JSON.stringify(d)));
  equal(await a.load(), d);
});
test("Additional heroes supported", () => {
  const d = base();
  for (let i = 0; i < 6; i++)
    d.children.push({
      id: "x" + i,
  ...
  equal(d.children.length, 8);
  equal(validateRoot(d), []);
  const s = createSession(d, "quick", 1000);
  equal(s.participants.length, 8);
});
test("English/Spanish localization", () => {
  const d = base();
  setLanguage("es");
  equal(t("start"), "Comenzar batalla");
  equal(localized(d.missions[0], "title"), "Pantano de Ropa");
  setLanguage("en");
  equal(localized(d.missions[0], "title"), "Clothing Swamp");
});
test("Service-worker project subdirectory paths", () => {
  const base = "/room-monster-battle/";
  const rel = "./js/app.js";
  assert(new URL(rel, "https://example.test" + base).pathname.startsWith(base));
});
...
for (const [n, f] of tests) {
  ...
  try {
    await f();
    li.textContent = "PASS — " + n;
    li.className = "pass";
    pass++;
  } catch (e) {
    li.textContent = "FAIL — " + n + ": " + e.message;
    li.className = "fail";
  }
  ol.append(li);
}
